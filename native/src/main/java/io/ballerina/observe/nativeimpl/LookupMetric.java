...
package io.ballerina.observe.nativeimpl;
        ...
        if (metric != null) {
            ...
            if (metric instanceof Counter) {
                BObject counter = ValueCreator.createObjectValue(
                        env.getCurrentModule(), ObserveNativeImplConstants.COUNTER,
                        StringUtils.fromString(metricId.getName()),
                        StringUtils.fromString(metricId.getDescription()), getTags(metricId));
                counter.addNativeData(ObserveNativeImplConstants.METRIC_NATIVE_INSTANCE_KEY, metric);
                return counter;
                ...
                BObject bGauge = ValueCreator.createObjectValue(
                        env.getCurrentModule(), ObserveNativeImplConstants.GAUGE,
                        StringUtils.fromString(metricId.getName()),
                        StringUtils.fromString(metricId.getDescription()), getTags(metricId), statisticConfigs);
                bGauge.addNativeData(ObserveNativeImplConstants.METRIC_NATIVE_INSTANCE_KEY, metric);
                return bGauge;
